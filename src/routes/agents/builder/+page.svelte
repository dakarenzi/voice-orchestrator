...
<script lang="ts">
  ...
  import { 
    Mic, Brain, Volume2, Globe, Save, ArrowLeft, 
    MessageSquare, Settings, GripVertical, Check, Activity 
  } from 'lucide-react';
  ...
  // Providers Palette
  const providers = [
    { type: 'stt', id: 'deepgram', name: 'Deepgram', icon: Mic, color: 'text-orange-600 bg-orange-100' },
    { type: 'llm', id: 'inworld', name: 'Inworld AI', icon: Brain, color: 'text-purple-600 bg-purple-100' },
    { type: 'llm', id: 'openai', name: 'OpenAI GPT-4', icon: Brain, color: 'text-green-600 bg-green-100' },
    { type: 'tts', id: 'elevenlabs', name: 'ElevenLabs', icon: Volume2, color: 'text-pink-600 bg-pink-100' },
    { type: 'tts', id: 'cartesia', name: 'Cartesia', icon: Volume2, color: 'text-blue-600 bg-blue-100' },
    { type: 'tts', id: 'google', name: 'Google TTS', icon: Volume2, color: 'text-red-600 bg-red-100' },
  ];
  ...
  function handleDrop(e: DragEvent, targetType: 'stt' | 'llm' | 'tts') {
    ...
    if (data) {
      ...
      if (provider.type === targetType) {
        ...
        const key = targetType === 'stt' ? 'sttProvider' : 
                   targetType === 'llm' ? 'llmProvider' : 'voiceProvider';
...
<div class="h-[calc(100vh-64px)] flex flex-col bg-slate-50 dark:bg-slate-900">
  <!-- Header -->
  <header class="h-16 border-b bg-background px-6 flex items-center justify-between shrink-0">
    <div class="flex items-center gap-4">
      <button class="hover:bg-accent p-2 rounded-full" on:click={() => goto('/agents')}>
        <ArrowLeft size={20} />
      </button>
      <div>
        <input 
    ...
    <div class="flex gap-2">
      <button class="px-4 py-2 bg-primary text-primary-foreground rounded-md flex items-center gap-2" on:click={handleSave}>
        {#if saving}
  ...
  <div class="flex-1 flex overflow-hidden">
    ...
    <aside class="w-64 border-r bg-background p-4 flex flex-col gap-6 overflow-y-auto">
      <div>
        <h3 class="text-sm font-semibold text-muted-foreground mb-4 uppercase tracking-wider">Providers</h3>
        <div class="space-y-3">
          {#each providers as p}
            <div 
              draggable="true"
      ...
      <div class="mt-auto p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-xs text-blue-700 dark:text-blue-300">
        <p class="font-semibold mb-1">Tip:</p>
        Drag providers from this list onto the pipeline slots in the center canvas to configure your agent.
      </div>
    </aside>
    ...
    <main class="flex-1 bg-slate-50 dark:bg-slate-950 relative overflow-y-auto">
      ...
      <div class="max-w-3xl mx-auto py-12 px-4 flex flex-col items-center gap-8 relative z-10">
        ...
        >
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <div class="text-xs text-muted-foreground">Name, Status, Metadata</div>
            </div>
          </div>
          <!-- Connection Line -->
        ...
        >
           <div class="flex items-center gap-4">
            <div class="h-10 w-10 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center group-hover:scale-110 transition-transform">
              <Mic size={20} />
            </div>
            <div>
              <div class="font-semibold">Input (STT)</div>
              <div class="text-xs text-primary font-medium capitalize">{$builderStore.agent.config?.sttProvider}</div>
            </div>
          </div>
          <div class="absolute -bottom-8 left-1/2 w-0.5 h-8 bg-slate-300"></div>
        </div>
        ...
        >
           <div class="flex items-center gap-4">
            <div class="h-10 w-10 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center group-hover:scale-110 transition-transform">
              <Brain size={20} />
            </div>
            <div>
              <div class="font-semibold">Brain (LLM)</div>
              <div class="text-xs text-primary font-medium capitalize">{$builderStore.agent.config?.llmProvider}</div>
            </div>
          </div>
          <div class="absolute -bottom-8 left-1/2 w-0.5 h-8 bg-slate-300"></div>
        </div>
        ...
        >
           <div class="flex items-center gap-4">
            <div class="h-10 w-10 rounded-full bg-pink-100 text-pink-600 flex items-center justify-center group-hover:scale-110 transition-transform">
              <Volume2 size={20} />
            </div>
            <div>
              <div class="font-semibold">Output (TTS)</div>
              <div class="text-xs text-primary font-medium capitalize">{$builderStore.agent.config?.voiceProvider}</div>
            </div>
          </div>
        </div>
    ...
    <aside class="w-80 border-l bg-background p-6 overflow-y-auto shadow-xl z-20">
      ...
      {:else if $builderStore.selectedNode === 'tts'}
        <TTSConfig />
      {:else}
        <div class="h-full flex flex-col items-center justify-center text-center text-muted-foreground p-4">
