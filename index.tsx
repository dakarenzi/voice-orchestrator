...
const SettingsView: React.FC = () => {
  ...
  const [orgName, setOrgName] = useState('My Voice Org');
  const [webhookUrl, setWebhookUrl] = useState('https://voiceorchestrator.com/api/webhooks/telnyx');
  const [notifications, setNotifications] = useState({ failure: true, summary: false });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={handleSave} className="gap-2"><Save size={16}/> {t('settings.saveAll')}</Button>
      </div>

      <div className="grid gap-6">
        <Card>
          <CardHeader>
             <h3 className="text-lg font-semibold flex items-center gap-2"><Globe size={18}/> {t('settings.orgConfig')}</h3>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
             <h3 className="text-lg font-semibold flex items-center gap-2"><LinkIcon size={18}/> Integrations</h3>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="outline" onClick={() => { navigator.clipboard.writeText(webhookUrl); addToast(t('common.copy'), 'success'); }}>
                  {t('common.copy')}
                </Button>
              </div>
              <p className="text-xs text-muted-foreground">{t('settings.webhookHint')}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
             <h3 className="text-lg font-semibold flex items-center gap-2"><Bell size={18}/> {t('settings.notifications')}</h3>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between p-3 border rounded-lg">
               <div>
                 <p className="font-medium">{t('settings.callFailure')}</p>
                 <p className="text-xs text-muted-foreground">{t('settings.callFailureHint')}</p>
               </div>
               <div className="flex items-center gap-2">
                  <span className="text-sm text-muted-foreground">{notifications.failure ? t('common.on') : t('common.off')}</span>
                  <button 
                    onClick={() => setNotifications(p => ({...p, failure: !p.failure}))}
                    className={`w-10 h-6 rounded-full p-1 transition-colors ${notifications.failure ? 'bg-primary' : 'bg-muted'}`}
                  >
                    <div className={`w-4 h-4 rounded-full bg-white transition-transform ${notifications.failure ? 'translate-x-4' : ''}`} />
                  </button>
               </div>
            </div>
            <div className="flex items-center justify-between p-3 border rounded-lg">
               <div>
                 ...
                 <p className="text-xs text-muted-foreground">{t('settings.dailySummaryHint')}</p>
               </div>
               <div className="flex items-center gap-2">
                  <span className="text-sm text-muted-foreground">{notifications.summary ? t('common.on') : t('common.off')}</span>
                   <button 
                    onClick={() => setNotifications(p => ({...p, summary: !p.summary}))}
                    className={`w-10 h-6 rounded-full p-1 transition-colors ${notifications.summary ? 'bg-primary' : 'bg-muted'}`}
                  >
                    <div className={`w-4 h-4 rounded-full bg-white transition-transform ${notifications.summary ? 'translate-x-4' : ''}`} />
                  </button>
               </div>
            </div>
          </CardContent>
...
const AnalyticsView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // Fallback for demo if API fails or is not available in preview environment
        let data;
        try {
            const res = await fetch(`/api/analytics/charts?range=${range}`);
            if (res.ok) {
                data = await res.json();
            } else {
            ...
            // Mock data fallback for preview resilience
            data = {
                dailyCalls: {
        ...
        setChartData({
          line: {
            labels: data.dailyCalls.labels,
            datasets: [
              {
                label: 'Total Calls',
                data: data.dailyCalls.data,
                fill: true,
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      {loading && <div className="flex justify-center p-12"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div></div>}
      ...
      {chartData && !loading && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="p-6">
            <h3 className="text-lg font-semibold mb-6">Call Volume Trend</h3>
            <div className="h-64 flex items-center justify-center">
               <Chart type='line' data={chartData.line} options={{ maintainAspectRatio: false, plugins: { legend: { display: false } } }} />
            </div>
          </Card>
          <Card className="p-6">
            ...
            <div className="h-64 flex items-center justify-center">
              <div className="w-2/3 h-full flex justify-center">
                <Chart type='doughnut' data={chartData.doughnut} options={{ maintainAspectRatio: false }} />
              </div>
            </div>
          </Card>
        </div>
        ...
        <CardContent>
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              ...
              <tr><td className="py-3">First Response Time</td><td className="py-3">1.2s</td><td className="py-3 text-green-500">-0.3s</td></tr>
            </tbody>
          </table>
        </CardContent>
      </Card>
